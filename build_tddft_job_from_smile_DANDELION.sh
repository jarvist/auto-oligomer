...
i=1
~/bin/smi2sdf -p ~/bin/mmxconst.prm -o "${name%.*}.sdf" "${name}" 
 babel -c -f $i -l $i "${name%.*}.sdf" -xb -xk "# opt uff" ${name%.*}_uff.com
...
%Mem=${MEMg}
%nproc=${CPUS}
#p sp guess=read geom=checkpoint scf=verytight b3lyp/GEN td(50-50,nstates=3)
